...
# tee command writes to sandard output (command line) and also multiple files (log files) 
...
VALIDATE(){
    if [ $1 -ne 0 ]
    then
        echo -e "$2 installation $R failed $N, please check further" | tee -a $LOG_FILE
        exit 1
    else 
        echo -e "$2 insatlled $G successfully $N" | tee -a $LOG_FILE
    fi
}
...
do
    ...
    then
        echo "$package does not exist, $package is installing now" | tee -a $LOG_FILE
        dnf isntall $package -y &>>$LOG_FILE
        VALIDATE $? "$package"   #calling VALIDATE function with 2 arguments
    else
        echo "$package is already isnatlled" | tee -a $LOG_FILE
    fi
done
